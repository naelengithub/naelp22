...
// Load react-p5 only on the client
const Sketch = dynamic(() => import("react-p5").then((mod) => mod.default), {
  ssr: false,
});

export default function InkCrystalShader3({ className = "" }) {
  ...
  const setup = (p5, canvasParentRef) => {
    ...
    p5.frameRate(30); // 🧩 Limit frame rate to avoid CPU spikes
    p5.clear();
    ...
    // 🎨 Keep your dark style
    screen.textFont("Satoshi, sans-serif");
    screen.background(51, 51, 51);
    screen.fill("#fcfcfa");
    screen.textSize(60);
